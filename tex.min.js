...
b.endsWith("\u3002")||(b+=PERIOD):r(),p.push(b))))}}r();abstractBox=["\\begin{abstract}","  "+g,"\\end{abstract}"];head.push("\\title{"+d+"}");head.push("\\begin{document}");head.push("  \\maketitle");0<g.length&&head.push(abstractBox.join("\n"));1<COLUMN&&head.push("\\begin{multicols}{"+COLUMN+"}");k.forEach(function(a){head.push(a)});1<COLUMN&&head.push("\\end{multicols}");head.push("\\end{document}");writer(d,head,window.PORT)})();
